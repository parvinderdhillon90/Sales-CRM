...
function initSchema(db: Database.Database) {
  db.exec(`
    ...
    -- Immutable audit trail - no UPDATE or DELETE allowed via application layer
...
function seedData(db: Database.Database) {
  ...
  const insertClient = db.prepare(
    `INSERT INTO clients (name, company, phone, email, address, zone, assigned_to, created_by, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  const insertDeal = db.prepare(
    `INSERT INTO deals (title, client_id, assigned_to, created_by, value, stage, temperature, expected_close_date, last_contact_date, last_contact_summary, close_date_change_count, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  ...
  const insertTask = db.prepare(
    `INSERT INTO tasks (title, description, created_by, assigned_to, deal_id, due_date, priority, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  );
  const insertMsg = db.prepare(
    `INSERT INTO messages (sender_id, recipient_id, subject, content, deal_id) VALUES (?, ?, ?, ?, ?)`
  );
  const insertTarget = db.prepare(
    `INSERT INTO targets (user_id, month, year, revenue_target, deal_count_target, created_by) VALUES (?, ?, ?, ?, ?, ?)`
  );

  const seed = db.transaction(() => {
    // --- USERS ---
    const rajesh = insertUser.run(
      'Rajesh', 'rajesh@salescrm.com',
      bcrypt.hashSync('Director@123', 10), 'director', null
    );
    const sunil = insertUser.run(
      'Sunil Balan', 'sunil@salescrm.com',
      bcrypt.hashSync('Sunil@123', 10), 'manager', 'south_west'
    );
    const sid = insertUser.run(
      'Siddharth Asija', 'siddharth@salescrm.com',
      bcrypt.hashSync('Siddharth@123', 10), 'manager', 'north'
    );
    ...
    const C1 = c1.lastInsertRowid as number;
    const C2 = c2.lastInsertRowid as number;
    const C3 = c3.lastInsertRowid as number;
    const C4 = c4.lastInsertRowid as number;
    ...
    const C6 = c6.lastInsertRowid as number;
    const C7 = c7.lastInsertRowid as number;
    const C8 = c8.lastInsertRowid as number;
    const C9 = c9.lastInsertRowid as number;
    ...
    // Deal 1: Hot negotiation deal - good progress
    const d1 = insertDeal.run(
      'Enterprise Software License - TechCorp', C1, S, R,
      250000, 'negotiation', 'hot',
      d(12), d(-2), 'Discussed pricing. They want 15% discount. Sent counter at 12%. Decision expected next week.',
      1, 'Deal initiated by Rajesh. Key enterprise account.'
    );
    ...
    // Deal 2: Warm proposal stage
    const d2 = insertDeal.run(
      'Retail Management System - Fashion Hub', C2, S, S,
      85000, 'proposal', 'warm',
      d(28), d(-6), 'Sent detailed proposal. Priya is reviewing with her CA. Should respond by end of week.',
      0, null
    );
    ...
    // Deal 3: Supply chain - warm but slightly stale
    const d3 = insertDeal.run(
      'Supply Chain Visibility Platform - Kerala Spices', C3, S, R,
      175000, 'qualified', 'warm',
      d(40), d(-17), 'Had intro meeting 17 days ago. Need to follow up on technical requirements doc they were supposed to send.',
      0, null
    );
    ...
    // Deal 4: Cold long-term lead
    const d4 = insertDeal.run(
      'IT Infrastructure Upgrade - Bangalore IT', C4, S, S,
      320000, 'lead', 'cold',
      d(75), d(-32), 'Cold call - client not urgent. Exploring for next FY budget. Follow up in 6 weeks.',
      0, null
    );
    ...
    // Deal 5: Won deal this month
    const d5 = insertDeal.run(
      'Pharma Compliance Suite - Hyderabad Pharma', C5, S, R,
      195000, 'closed_won', 'hot',
      d(-5), d(-5), 'CONTRACT SIGNED! Full implementation begins next month.',
      0, 'Successfully closed. Reference client for pharma vertical.'
    );
    ...
    // Deal 6: OVERDUE — past close date, high value, stale (showing biggest problem)
    const d6 = insertDeal.run(
      'ERP Full Implementation - Delhi Enterprises', C6, SI, R,
      580000, 'proposal', 'hot',
      d(-8), d(-22), 'Client asked for revised proposal 22 days ago. No follow up done. CRITICAL DEAL AT RISK.',
      2, 'Biggest deal in pipeline. Rajesh\'s account. Must close this quarter.'
    );
    ...
    // NO ACTIVITY IN LAST 22 DAYS — this is the problem!

    // Deal 7: Stale - family business complexity
    const d7 = insertDeal.run(
      'Food Processing ERP - Punjab Foods', C7, SI, SI,
      145000, 'qualified', 'warm',
      d(18), d(-28), 'Met with Sunita 28 days ago. Need to get all 3 family members together for next meeting. No response to 2 follow-up emails.',
      1, null
    );
    ...
    // 28 days stale — problem!

    // Deal 8: Competitor risk - price sensitive
    const d8 = insertDeal.run(
      'Textile ERP - UP Textile Mills', C8, SI, SI,
      95000, 'lead', 'cold',
      d(85), d(-48), 'Very price sensitive. Currently evaluating 4 other vendors. Long sales cycle.',
      0, null
    );
    ...
    // Deal 9: HOT - about to close
    const d9 = insertDeal.run(
      'HR Analytics Suite - HR Solutions Delhi', C9, SI, R,
      210000, 'negotiation', 'hot',
      d(6), d(-3), 'Final terms discussion ongoing. Neha wants 10% discount on implementation. Very close to signing.',
      0, 'Rajesh referred. Fast-growing HR tech company. Strong reference case if won.'
    );
    ...
    // Deal 10: Ghost deal - no expected close date, very old, cold
    const d10 = insertDeal.run(
      'Mineral Export Solution - Rajasthan Minerals', C10, SI, SI,
      67000, 'lead', 'cold',
      null, d(-65), 'First contact only. No follow up done. Expected close date never set.',
      0, null
    );
    ...
    // 65 days stale, no expected close date set!

    // =====================================================
    // TASKS
    // =====================================================

    // Overdue tasks (Siddharth)
    ...
    insertTarget.run(S, month, year, 600000, 3, R);
    insertTarget.run(SI, month, year, 900000, 4, R);
  });
